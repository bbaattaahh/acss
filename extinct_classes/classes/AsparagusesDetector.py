...
class AsparagusesDetector(object):
    def __init__(self,
                 cascade_file,
        ...
        self.asparagus_cascade = cv2.CascadeClassifier(cascade_file)
    ...
    def data_to_analysis_one_asparagus_images(self, image):
        ...
        for candidate in self.asparagus_candidates(image_detection_on):
    ...
    def asparagus_candidates(self, image_detection_on):
        detections = []

        for angle in range(-self.swing_angle, self.swing_angle + 1):
            rotated_image_detection_on = self.rotate_about_center(image_detection_on, angle)
            actual_asparagus_candidates = self.asparagus_cascade.detectMultiScale(rotated_image_detection_on, 4, 31)
            if len(actual_asparagus_candidates) != 0:
                for actual_asparagus_candidate in actual_asparagus_candidates:
                    rectangle = Rectangle(top_left_x=actual_asparagus_candidate[0].tolist(),
                                          ...
                                          angle=0)
                    is_rectangle_on_original_image = IsRectangleOnOriginalImage(image_detection_on,
